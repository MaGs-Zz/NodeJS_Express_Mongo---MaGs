...
const Cursos = () => {
    ...
    const fetchUsuariosInscritos = async (cursoId) => {
        try {
            const response = await fetch(`https://localhost:3000/api/cursos/${cursoId}/usuarios`);
    ...
    const handleUpdateCurso = async () => {
        try {
            const response = await fetch(`https://localhost:3000/api/cursos/${selectedCurso._id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok) {
                ...
                setCursos(
                    cursos.map(curso => (curso._id === selectedCurso._id ? cursoActualizado : curso))
                );
    ...
    const handleDeleteCurso = async () => {
        ...
        try {
            const response = await fetch(`https://localhost:3000/HTTP://localhost:3000/api/cursos/${selectedCurso._id}`, {
                method: 'DELETE',
            });
        ...
        <Container>
                        ...
                        {cursos.map(curso => (
                            <TableRow key={curso._id}>
                                ...
                                <TableCell align="right">
                                    <IconButton onClick={() => handleEditClick(curso)} color="primary">
                                        <Edit />
                                    </IconButton>
                                    <IconButton onClick={() => handleInfoClick(curso)} color="info">
                                        <Info />
                                    </IconButton>
                                    <IconButton onClick={() => handleDeleteClick(curso)} color="secondary">
                                        <Delete />
                                    </IconButton>
                                </TableCell>
                            </TableRow>
            ...
            <Dialog open={openInfoDialog} onClose={handleCloseInfoDialog} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers>
                    <Typography variant="h6">Información del Curso</Typography>
                    <Typography>Título: {selectedCurso.titulo}</Typography>
                    <Typography>Descripción: {selectedCurso.descripcion}</Typography>
                    <Typography>Calificación: {selectedCurso.calificacion}</Typography>
                    <Typography>Estado: {selectedCurso.estado ? 'Activo' : 'Inactivo'}</Typography>
                    <Typography>Número de Alumnos: {selectedCurso.alumnos}</Typography>
                    <Typography>Imagen:</Typography>
                    <img
                        src={selectedCurso.imagen || defaultImage}
                        alt="Imagen del curso"
                        width="100"
                    />
                    <Box marginTop={2}>
                        ...
                        {usuariosInscritos.length > 0 ? (
                            ...
                            <Typography>No hay usuarios inscritos en este curso.</Typography>
                        )}
                    </Box>
                </DialogContent>
                <DialogActions>
            ...
            <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog} maxWidth="xs" fullWidth>
                ...
                <DialogContent dividers>
                    <Typography>
                        ¿Estás seguro de que quieres eliminar el curso <strong>{selectedCurso?.titulo}</strong>?
                    </Typography>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseDeleteDialog} color="default">Cancelar</Button>
