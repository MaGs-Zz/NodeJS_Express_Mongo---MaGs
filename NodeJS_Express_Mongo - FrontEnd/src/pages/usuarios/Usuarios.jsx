...
import {
    ...
    TableRow, Paper,
    IconButton, Switch, Box, Dialog, DialogTitle, DialogContent, DialogActions,
    Typography, Snackbar, Alert, Checkbox, List, ListItem, ListItemText
} from '@mui/material';
...
const Usuarios = () => {
    // Estados para gestionar la lista de usuarios, cursos y otros datos
    ...
    const [openDeleteDialog, setOpenDeleteDialog] = useState(false); // Controla si el diálogo de eliminación está abierto
    const [openInfoDialog, setOpenInfoDialog] = useState(false); // Controla si el diálogo de información está abierto
    const [openAddCursosDialog, setOpenAddCursosDialog] = useState(false); // Controla si el diálogo para agregar cursos está abierto
    ...
    const [openSnackbar, setOpenSnackbar] = useState(false); // Controla si el snackbar (notificación) está abierto
    const defaultImage = 'https://via.placeholder.com/50'; // URL de una imagen de placeholder

    // Efecto para cargar los usuarios y cursos al inicializar el componente
    useEffect(() => {
        fetchUsuarios();
    ...
    const fetchUsuarios = async () => {
        try {
            ...
            setError(error, 'Error al obtener usuarios');
        }
    };
    ...
    const handleUpdateUsuario = async () => {
        ...
        // Remove the 'email' from formValues if it exists
        const { email, ...updatedValues } = formValues;
    
        try {
            const response = await fetch(`https://localhost:3000/api/usuarios/${selectedUsuario.email}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(updatedValues),  // Send updatedValues without 'email'
            });
    ...
    const handleDeleteUsuario = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new new Error(errorData.error || 'Error al eliminar usuario');
            }
    ...
    const handleInfoClick = async (usuario) => {
        ...
        try {
            const response = await fetch(`https://localhost:3000/api/usuarios/${usuario._id}/cursos`);
    ...
    const handleAddCursosClick = async (usuario) => {
        ...
        try {
            const response = await fetch(`https://localhost:3000/api/usuarios/${usuario._id}/cursos`);
            if (!response.ok) throw new Error('Error al obtener los cursos del usuario');

            // Verificar que la respuesta tiene contenido antes de convertir a JSON
            const textResponse = await response.text();
            const cursosUsuario = textResponse ? JSON.parse(textResponse) : [];
    ...
    const handleSaveCursos = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const textResponse = await response.text();
                const errorData = textResponse ? JSON.parse(textResponse) : { error: 'Error desconocido' };
